...
const Checkerboard:React.FC<BoardProps> = ({boardSize}) => {
  ...
  const initSquares = Array.from({length: boardSize}, ()=>new Array(boardSize).fill(''))
  ...
  const jumpToStep = (step:number) => {//跳到指定步骤
    ...
    setTimes(step)
    setWinner('')
    const gameWinner = calculateWinner(history[step], (boardSize !== 3) ? 5 : 3)
    if(gameWinner) {
  ...
  return (
    <div>
      ...
      {!winner && <div className="flex-cc mt-20">下一个玩家： <div className='w-20'>{nextUser}</div></div>}
    </div>
  )
}
